...
Object Descriptor Buffer Entry limit Write Port. +
Object Descriptor Buffer Entry base write Port. +
Object Descriptor Buffer Entry limit Read Port. +
Object Descriptor Buffer Entry base Read Port.
|===
...
* TSetDescriptor指令，将cs1作为base，rs2作为limit，生成标记指针写入cd，描述符写入默认csr寄存器对（odbbasewrite, odblimitwrite)中。
进行标记检查、边界检查，保证[cs1, rs2]对象不超过cs1对象的边界。

* TODBInsert指令，将指定的csr寄存器对（odbbasewrite, odblimitwrite）写入ODB中。
