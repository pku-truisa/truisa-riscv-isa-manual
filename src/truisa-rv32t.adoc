...
* TSetDesc指令，将cs1作为base，rs2作为长度limit，生成S-Type标记指针写入cd。base和limit须符合单调性原则。
...
1. 用CSetAddr生成新的base保存到cs1 和 计算bound-base+1的limit值保存到rs2寄存器中。

2. 使用TSetDesc指令生成S-Type的标记指针, cs1是对象的base，rs2是对象的长度limit，cd是对象的标记指针。保证生成的内存对象是cs1内存对象的子集。
