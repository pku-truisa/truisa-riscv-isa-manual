...
|Object Descriptor Buffer Entry Bound. +
...
13+^|*RV32T Base Instruction Set*
 ...
 2+^|0001010        2+^|00000 2+^|cs1   2+^|000    2+^|00000        2+^| 0001111 <|TODBRW
...
* TODBRW指令，将指定的csr寄存器对（odbentrybase, odbentrybound）写入ODB中，cs1作为标记指针提供S-Radix。

* TFENCE.VMA指令，flush ODB，cs1如果是全0，表示flush所有表项，否则flush cs1指示的对象表项。