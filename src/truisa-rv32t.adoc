...
|===
  26+^|*RV32T标记指针格式 (24比特线性地址）*
   ...
   8+^|0x00                      18+^|Address   
   1+^|0 1+^|W 1+^|0 5+^|S-Radix 18+^|Address    
   1+^|1 1+^|W 1+^|0 5+^|B-Radix 18+^|Address 
   8+^|0xFF                      18+^|Address
|===
...
|===
  26+^|*RV64T标记指针格式（56比特线性地址）*
   ...
   8+^|0x00                         18+^|Address
   1+^|0 1+^|W 6+^|S-Radix          18+^|Address 
   1+^|1 1+^|W 6+^|B-Radix          18+^|Address
   8+^|0xFF                         18+^|Address
|===
...
* S-Radix表示Address所在内存对象的Centroid标识是2的S-Radix幂次方大小和对齐的内存块中间点地址。S-Radix大于0，小于等于24，如果大于25的值都按照24处理。
* B-Radix表示Address所在内存对象是固定大小的内存块，块大小和对齐都是2的B-Radix幂次方。B-Radix小于等于25，大于24的值都按24处理。
...
|===
  26+^|*RV32T描述符旁视缓冲表项格式*
   ...
   1+^|L 1+^|G 3+^|000   1+^|X 1+^|0 1+^|R 18+^|Base Address
   1+^|0 1+^|W 6+^|S-Radix                 18+^|Bound Address
...
* CSetBounds/CSetBoundImm，cs1作为base地址，rs2是对象长度，生成大于rs2长度的最小B-Radix作为cd的B-Radix，W位相同。
